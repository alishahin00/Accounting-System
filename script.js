...
// Run on init — assign journal numbers to any legacy rows
...
(function loadOpeningBalances() {
  try {
    openingBalances = JSON.parse(localStorage.getItem('acc_opening_balances')) || {};
...
function renderBalanceSheet() {
  ...
  const totalRev = rawTB.filter(r => getCodeGroup(r) === 'Revenue').reduce((s, r) => s + (r.credit - r.debit), 0);
  const totalExp = rawTB.filter(r => getCodeGroup(r) === 'Expense').reduce((s, r) => s + (r.debit - r.credit), 0);
  const netIncome = totalRev - totalExp;

  function fillSection(tbodyId, type, calcFn) {
    const tbody = document.getElementById(tbodyId);
    tbody.innerHTML = '';
    const tree = buildFlatTree(a => a.type === type && hasBalance.has(a.id));
    let leafTotal = 0;
  ...
  const totalLiab = fillSection('bs-liabilities-tbody', 'Liability', v => v.credit - v.debit);
  let totalEq = fillSection('bs-equity-tbody', 'Equity', v => v.credit - v.debit);

  // Add retained earnings (net income) row to equity
  if (netIncome !== 0) {
    const eqBody = document.getElementById('bs-equity-tbody');
    eqBody.innerHTML += `
      <tr>
        <td style="padding:10px 14px;font-style:italic">Retained Earnings (Net Income)</td>
        <td class="col-money" style="padding:10px 14px">${formatMoney(netIncome)}</td>
      </tr>`;
    totalEq += netIncome;
  }

  document.getElementById('total-assets').textContent = formatMoney(totalAssets);
  document.getElementById('total-liabilities').textContent = formatMoney(totalLiab);
  document.getElementById('total-equity').textContent = formatMoney(totalEq);

  // Combined Liabilities & Equity total
  document.getElementById('total-liab-equity-combined').textContent = formatMoney(totalLiab + totalEq);

  // Equation card
  document.getElementById('eq-assets').textContent = formatMoney(totalAssets);
  document.getElementById('eq-liab-equity').textContent = formatMoney(totalLiab + totalEq);

  const eqStatus = document.getElementById('eq-status');
  const balanced = Math.abs(totalAssets - (totalLiab + totalEq)) < 0.005;
  if (balanced) {
    eqStatus.textContent = '✓ Balanced';
    eqStatus.className = 'eq-status balanced';
  } else {
...
document.getElementById('btn-export-balance').addEventListener('click', () => {
  const tb = computeTrialBalance();
  const rows = [['Category', 'Account Name', 'Amount']];
  tb.filter(r => r.type === 'Asset').forEach(r => rows.push(['Asset', r.name, r.debit - r.credit]));
  tb.filter(r => r.type === 'Liability').forEach(r => rows.push(['Liability', r.name, r.credit - r.debit]));
  tb.filter(r => r.type === 'Equity').forEach(r => rows.push(['Equity', r.name, r.credit - r.debit]));
